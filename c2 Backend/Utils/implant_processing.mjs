...
const exe_processing =async(impl_path="", url, port, group, sys)=>{
    ...
    try {
        ...
        if (sys === 'windows') {
            ...
            const replace = await runPowerShellCommand([`python`, path.normalize('./scripts/append_text_windows.py'), '-f', copy_path, '-t', 'write', '-u', url, '-p', port, '-g', group]);
            ...
            const copy = await runPowerShellCommand([`copy`, impl_path, copy_path ]);
            const replace = await runPowerShellCommand([`python`, path.normalize('./scripts/append_text_linux.py'), '-f', copy_path, '-t', 'write', '-u', url, '-p', port, '-g', group]);
            result_path=`..\\${copy_path}`
